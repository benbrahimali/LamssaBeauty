...
import '../widgets/closure_dialog.dart';
...
class _CaisseScreenState extends State<CaisseScreen> {
  ...
  Future<void> _closeDay() async {
    ...
    final saisie = await ClosureDialog.show(context, tresor?.expectedCash ?? 0);
    if (saisie == null || !mounted) return;

    final closure = await controller.closeDay(
      countedCash: saisie.countedCash,
      withdrawal: saisie.withdrawal,
      varianceReason: saisie.reason,
    );
    if (!mounted) return;
